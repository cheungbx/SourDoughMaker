...
const char* release_version = "1.5";
...
enum InstructionType { TYPE_COLOUR, TYPE_KNEAD, TYPE_DEGAS, TYPE_PROOF, TYPE_REST, TYPE_BAKE, TYPE_WEIGHT, TYPE_MENU };
...
long calculateTotalDurationSec() {
  ...
  for (int i = 0; i < instructionCount; i++) {
    if (instructions[i].type != TYPE_COLOUR && instructions[i].type != TYPE_WEIGHT && instructions[i].type != TYPE_MENU) {
...
void loop() {
  ...
  if (currentMillis - previousMillis >= 1000UL) {
    ...
    if (currentStepRemainingSec > 0 && instructions[currentStepIdx].type != TYPE_COLOUR && instructions[currentStepIdx].type != TYPE_WEIGHT && instructions[currentStepIdx].type != TYPE_MENU) {
