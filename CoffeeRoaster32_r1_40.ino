...
#include <SPIFFS.h>        // Flash File System for logging

const String RELEASE_VERSION = "1.40"; // INCREMENTED: Release version bump
...
Killswitch    K4      GPIO 4

Servo's       Brown   GND
servo's       Red     3.3V
Servo's       Orange  Singal

Arduino Nano  10-pin Female header Coffee Roasting Machine   
GND           pin 1         GND
VIN           pin 2         5V
D9            pin 3         Temperature ROTARY encoder
D8            pin 4         Temperature ROTARY encoder Quadrature Signal
D7            pin 5         Time ROTARY encoder
D6            pin 6         Time ROTARY encoder Quadrature Signal
D5            pin 7         OneOff button
D4            pin 8         Fan button
N.C.          pin 9
N.C.          pin 10

Arduino Nano  ESP32  
A0            GPIO 25       TemperaturePin Rotary Encoder
A1            GPIO 16       TemperatureAPin Rotary Encoder Quadrature Signal
A2            GPIO 23       TimePin Rotry Encoder
A3            GPIO 17       TimeAPin Rotary Encoder Quadrature Signal
A4            GPIO 18       OnOffPin
A5            GPIO 19       FanPin 
...
const bool ServoMode = false; // Release 1.37: Set to false to drive roasting machine via digital IO directly

// --- ESP32 Hardware Pins ---
const int TemperaturePin  = 25; 
const int TemperatureAPin = 16; // GPIO assigned for Temperature Quadrature Signal
const int TimePin         = 23; 
const int TimeAPin        = 17; // GPIO assigned for Time Quadrature Signal
...
// --- Charting & Flash Excel Storage Variables ---
...
volatile bool triggerWebCooldown = false;
...
// --- SPIFFS Excel Logger Routines ---
void eraseRoastLogFile() {
  if (SPIFFS.exists("/roast_log.xls")) {
    SPIFFS.remove("/roast_log.xls");
  }
  File f = SPIFFS.open("/roast_log.xls", FILE_WRITE);
  if (f) {
...
void logRoastDataPoint() {
  File f = SPIFFS.open("/roast_log.xls", FILE_APPEND);
  if (f) {
    ...
    f.printf("%s\t%d\t%.2f\t%.2f\n", getFormattedTime((int)elapsedSecs).c_str(), targetTemp, pt100Temp, currentRoR);
    f.close();
  }
}

// --- Digital and Servo Control Routines ---
void executeServoPress(Servo &srv, int degree) {
  srv.write(degree);
  delay(pressTime);
  srv.write(0);
...
void executeDigitalPress(int pin) {
  if (DebugLevel >= 1) {  
    Serial.printf("[%s] [Sequence] Digital Press Pin: %d \n", getFormattedTime(totalRemainingTimeSec).c_str(), pin);
  }   
...
// Quadrature simulation logic helper
void simulateEncoderTurn(int pinLead, int pinLag, int nbrOfTicks) {
  if (DebugLevel >= 1) {  
    Serial.printf("[%s] [Sequence] Encoder Turn Lead Pin: %d, Lag Pin: %d, ticks: %d\n", getFormattedTime(totalRemainingTimeSec).c_str(), pinLead, pinLag, nbrOfTicks);
  }   
  if (nbrOfTicks == 0) return;
  int pa = (nbrOfTicks > 0) ? pinLead : pinLag;
  int pb = (nbrOfTicks < 0) ? pinLead : pinLag;
  
  for (int i = 0; i < abs(nbrOfTicks); i++) {
    digitalWrite(pa, LOW);
    delay(5);
    digitalWrite(pb, LOW);
    delay(5);
...
void setMaximumTime() {
  if (ServoMode) {
    ...
    if (DebugLevel >= 1) {
      Serial.printf("[%s] [Hardware] Simulating Time Encoder CW Max (120 ticks\n", getFormattedTime(totalRemainingTimeSec).c_str());
    }
    simulateEncoderTurn(TimePin, TimeAPin, 120);
  }
}
...
void setMinimumTime() {
  if (ServoMode) {
    ...
    if (DebugLevel >= 1) {
      Serial.printf("[%s] [Hardware] Simulating Time Encoder CCW Min (-120 ticks)\n", getFormattedTime(totalRemainingTimeSec).c_str());
    }
    simulateEncoderTurn(TimePin, TimeAPin, -120);
  }
}
...
void AdjustTemperatureDigital(int targetTemperature) {
   simulateEncoderTurn (TemperaturePin, TemperatureAPin, targetTemperature - lastTemperature);
  lastTemperature = targetTemperature;
}

void adjustTemperature(int targetTemp) {
     if (DebugLevel >= 1) {
          Serial.printf("[%s] [Hardware] Adjust to targetTemp %dC from lastTemp %dC.\n", getFormattedTime(totalRemainingTimeSec).c_str(), targetTemp,  lastTemperature);
       }
...
// Direct immediate execution logic for Kill Switch & Web Cooldown trigger
void triggerImmediateCooldown() {
  awaitingCoolDownConfirm = false;
  if (DebugLevel >= 1) {
    Serial.printf("[%s] [Cooldown Command] Bypassing remaining profile steps straight to Cooldown.\n", getFormattedTime(totalRemainingTimeSec).c_str());
  }
  
  if (ServoMode) executeServoPress(ServoOnOff, OnOffAngle); 
  else executeDigitalPress(OnOffPin);
...
// --- Dynamic Reboot Page Generation to fix iOS Safari Reconnection issues ---
String generateRebootingHtml(String message, int seconds) {
  String html = "<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width, initial-scale=1.0'>";
  html += "<title>Billy Roaster - Rebooting</title>";
  html += "<style>body { font-family: Arial, sans-serif; text-align: center; margin-top: 50px; background-color: #faf8f5; color: #3e2723; }";
  html += ".card { background: white; padding: 30px; max-width: 380px; margin: auto; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); border-top: 5px solid #5d4037; }";
  html += ".timer { font-size: 32px; font-weight: bold; color: #795548; margin: 20px 0; }</style>";
  html += "<script>";
  html += "var count = " + String(seconds) + ";";
  html += "var interval = setInterval(function() {";
  html += "  count--;";
  html += "  if(count > 0) {";
  html += "    document.getElementById('cnt').innerText = count;";
  html += "  } else {";
  html += "    clearInterval(interval);";
  html += "    document.getElementById('status').innerText = 'Reconnecting to page...';";
  html += "    checkConnection();";
  html += "  }";
  html += "}, 1000);";
  html += "function checkConnection() {";
  html += "  fetch('/', { method: 'HEAD', cache: 'no-store' }).then(function(response) {";
  html += "    if (response.ok) { window.location.href = '/'; }";
  html += "    else { setTimeout(checkConnection, 1500); }";
  html += "  }).catch(function(error) {";
  html += "    setTimeout(checkConnection, 1500);";
  html += "  });";
  html += "}";
  html += "</script></head><body><div class='card'>";
  html += "<h2>" + message + "</h2>";
  html += "<p id='status'>Device is rebooting. Please wait...</p>";
  html += "<div class='timer'><span id='cnt'>" + String(seconds) + "</span>s</div>";
  html += "</div></body></html>";
  return html;
}

String generateRestartingHtml(const String& message, int delaySeconds = 5) {
  String html = "<!DOCTYPE html><html><head>";
  html += "<meta name='viewport' content='width=device-width, initial-scale=1.0'>";
  html += "<meta http-equiv='refresh' content='" + String(delaySeconds) + ";url=/'>";
  html += "<title>Restarting...</title>";
  html += "<style>";
  html += "body { font-family: Arial, sans-serif; text-align: center; margin-top: 50px; background-color: #f7f9fa; }";
  ...
  html += "</style>";
  html += "<script>";
  html += "var seconds = " + String(delaySeconds) + ";";
  html += "setInterval(function() {";
  html += "  seconds--;";
  html += "  if (seconds >= 0) document.getElementById('timer').innerText = seconds;";
  html += "}, 1000);";
  html += "</script>";
  html += "</head><body><div class='card'>";
  html += "<h2>" + message + "</h2>";
  html += "<p>Reconnecting to device in <b id='timer'>" + String(delaySeconds) + "</b> seconds...</p>";
  html += "<p><a href='/'>Click here if you are not redirected automatically.</a></p>";
  html += "</div></body></html>";
  return html;
}
...
String generateHtml() {
  ...
  html += "button.cooldown-init { background-color: #00838f; }";
  html += "button.cooldown-confirm { background-color: #d32f2f; font-weight: bold; }";
  ...
  html += "}";
  html += "function exposeCooldownConfirmButton() {";
  html += "  document.getElementById('cooldownInitBtn').style.display = 'none';";
  html += "  document.getElementById('cooldownConfirmBtn').style.display = 'inline-block';";
  html += "}";
  html += "</script>";
  html += "</head><body><div class='card'>";
  ...
  if (currentState == MENU_SELECTION) {
    if (!isConfirmed) {
      if (ServoMode) {
        ...
        html += "<button class='servo-btn' onclick='location.href=\"/digital_setup\"'>Interface Test</button><br>";
      }
    }
    ...
    if (currentState == Roasting) {
      html += "<button id='cooldownInitBtn' class='cooldown-init' onclick='exposeCooldownConfirmButton()'>Cooldown</button>";
      html += "<button id='cooldownConfirmBtn' class='cooldown-confirm' style='display:none;' onclick='location.href=\"/cooldown\"'>Confirm to Cooldown</button><br>";
    }
  }

  // Button to allow log download
  html += "<button class='download' onclick='location.href=\"/download_log\"'>Download Chart</button><br>";
...
// --- Digital Interface Test Setup Page HTML Generator ---
String generateDigitalSetupHtml() {
  String html = "<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width, initial-scale=1.0'>";
  html += "<title>Interface Test " + RELEASE_VERSION + "</title>";
  html += "<style>";
  ...
  html += "button { background-color: #795548; color: white; border: none; padding: 8px 14px; cursor: pointer; border-radius: 4px; margin: 4px; font-size: 14px; }";
  html += "button.return-btn { background-color: #757575; padding: 10px 20px; font-size: 16px; margin-top: 15px; }";
  ...
  html += "<h2>Interface Test</h2>";
  html += "<p><strong>Status:</strong> Interface Test</p><hr>";

  html += "<div class='row'><span>OnOff:</span><button type='button' onclick='fetch(\"/test_digital?act=onoff\")'>Test</button></div>";
  html += "<div class='row'><span>Fan:</span><button type='button' onclick='fetch(\"/test_digital?act=fan\")'>Test</button></div>";
  html += "<div class='row'><span>Minimum Time:</span><button type='button' onclick='fetch(\"/test_digital?act=min_time\")'>Test</button></div>";
  html += "<div class='row'><span>Maximum Time:</span><button type='button' onclick='fetch(\"/test_digital?act=max_time\")'>Test</button></div>";

  for (int temp = 150; temp <= 240; temp += 10) {
    html += "<div class='row'><span>" + String(temp) + "C:</span><button type='button' onclick='fetch(\"/test_digital?act=temp&val=" + String(temp) + "\")'>Test</button></div>";
  }

  html += "<br><div style='text-align:center;'>";
  html += "<button type='button' class='return-btn' onclick='location.href=\"/cancel_servo_setup\"'>Return</button>";
...
String generateChartHtml() {
  ...
  // Read data points from SPIFFS TSV (Excel Log)
  int times[300], targets[300];
  float stoves[300], rors[300];
  int dataCount = 0;

  File f = SPIFFS.open("/roast_log.xls", FILE_READ);
  if (f) {
    f.readStringUntil('\n'); // Skip header
    while (f.available() && dataCount < 300) {
      ...
      if (line.length() > 0) {
        ...
        if (c1 != -1 && c2 != -1 && c3 != -1) {
          ...
          if (colon != -1) {
            times[dataCount] = (timeStr.substring(0, colon).toInt() * 60) + timeStr.substring(colon + 1).toInt();
          } else {
            times[dataCount] = timeStr.toInt();
          }
...
void updateOledDisplay() {
  ...
  // --- Servo / Interface Setup Mode Screen Layout ---
  if (inServoSetupMode) {
    ...
    if (ServoMode) {
      ...
      display.println("Status: Interface Test");
    }
    display.display();
    return;
  }
  ...
  // --- Roasting State Specific Layout ---
  if (currentState == Roasting) {
    // Lines 5-8: Current and next 3 instructions
    for (int i = 0; i < 4; i++) {
      int idx = currentInstructionIdx + i;
      if (idx < instructionCount) {
        int displayTimeSec = (i == 0) ? currentRemainingTimeSec : instructions[idx].timeInSeconds;
        char lineBuf[32];
        snprintf(lineBuf, sizeof(lineBuf), "%d/%d %s %dC FAN %d", 
                 idx + 1, 
                 instructionCount, 
                 getFormattedTime(displayTimeSec).c_str(), 
                 instructions[idx].temperature, 
                 instructions[idx].fanSpeed);
...
void setup() {
  ...
  server.on("/digital_setup", HTTP_GET, [](AsyncWebServerRequest *request){
    inServoSetupMode = true;
    updateOledDisplay();
    request->send(200, "text/html", generateDigitalSetupHtml());
  });
  ...
  server.on("/download_log", HTTP_GET, [](AsyncWebServerRequest *request){
    if (SPIFFS.exists("/roast_log.xls")) {
      request->send(SPIFFS, "/roast_log.xls", "application/vnd.ms-excel", true);
    } else {
  ...
  server.on("/test_digital", HTTP_GET, [](AsyncWebServerRequest *request){
    if (request->hasParam("act")) {
      ...
      if (act == "onoff") {
        ...
        executeDigitalPress(FanPin);
      } else if (act == "min_time") {
        setMinimumTime();
      } else if (act == "max_time") {
        setMaximumTime();
      } else if (act == "temp" && request->hasParam("val")) {
        int tVal = request->getParam("val")->value().toInt();
        adjustTemperature(tVal);
      }
    }
    request->send(200, "text/plain", "OK");
  });
  ...
  server.on("/cooldown", HTTP_GET, [](AsyncWebServerRequest *request){
    if (currentState == Roasting) {
      triggerWebCooldown = true;
    }
    request->redirect("/");
  });
...
server.on("/reset", HTTP_GET, [](AsyncWebServerRequest *request){
    triggerReset = true; 
    request->send(200, "text/html", generateRestartingHtml("Device is restarting..."));
  });
  ...
  server.on("/erase_all", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    request->send(200, "text/html", generateRestartingHtml("Settings erased. Restarting..."));
  });
  ...
  server.on("/switch_mode", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    if (request->hasParam("to")) {
      ...
      if (target == "wifi") {
        ...
        request->send(200, "text/html", generateRestartingHtml("Switching to WIFI Mode..."));
        return;
      } else if (target == "standalone") {
        targetSwitchMode = MODE_STANDALONE;
        triggerSwitchMode = true;
        request->send(200, "text/html", generateRestartingHtml("Switching to Standalone Mode..."));
        return;
      }
    }
    request->redirect("/");
  ...
  server.on("/save_config", HTTP_POST, [](AsyncWebServerRequest *request){
    if (currentState == WIFI_CONFIG_AP) {
      ...
      request->send(200, "text/html", generateRestartingHtml("Configuration saved. Restarting..."));
      return;
    }
    request->redirect("/");
  });
...
void loop() {
 if (triggerReset) {
    triggerReset = false;
    delay(1000); // Allow the AsyncWebServer to send the page before rebooting
    ESP.restart(); 
  }
  ...
  if (triggerWebCooldown) {
    triggerWebCooldown = false;
    if (currentState == Roasting) {
  ...
  if (triggerTestOnOff) {
    ...
    if (ServoMode) executeServoPress(ServoOnOff, testServoVal);
    else executeDigitalPress(OnOffPin);
  }
  if (triggerTestFan) {
    triggerTestFan = false;
    if (ServoMode) executeServoPress(ServoFan, testServoVal);
    else executeDigitalPress(FanPin);
  }
  if (triggerTestMin) {
