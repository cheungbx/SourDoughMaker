...
const char* release_version = "1.8";
...
void updateOLEDDisplay() {
  ...
  // Line 1: Header - Title and Version
  ...
  if (currentState == RUNNING_STEP) {
    // Line 3: Running status + Total remaining time
    display.setCursor(0, 12);
    display.printf("Running %s", getFormattedTimeHMS(calculateTotalRemainingSec()).c_str());

    // Lines 4-7: Up to 4 rolled instruction steps (Current step + next 3)
    int lineY = 24;
    for (int i = 0; i < 4; i++) {
      int stepIdx = currentStepIdx + i;
      if (stepIdx < instructionCount) {
        display.setCursor(0, lineY);
        ProfileInstruction inst = instructions[stepIdx];

        if (stepIdx == currentStepIdx) {
          // Current instruction uses live remaining time
          if (inst.type == TYPE_COLOUR) {
            display.printf("%d/%d %s %s", stepIdx + 1, instructionCount, inst.functionName.c_str(),
                           (inst.colourOption == 0 ? "Light" : (inst.colourOption == 1 ? "Medium" : "Dark")));
          } else if (inst.type == TYPE_WEIGHT) {
            display.printf("%d/%d %s %s", stepIdx + 1, instructionCount, inst.functionName.c_str(),
                           (inst.weightOptionValue == 0 ? "Light" : "Heavy"));
          } else if (inst.type == TYPE_MENU) {
            display.printf("%d/%d Menu %d", stepIdx + 1, instructionCount, inst.menuNumber);
          } else if (inst.type == TYPE_STOP) {
            display.printf("%d/%d Stop", stepIdx + 1, instructionCount);
          } else {
            display.printf("%d/%d %s %s", stepIdx + 1, instructionCount, inst.functionName.c_str(), getFormattedTimeHMS(currentStepRemainingSec).c_str());
          }
        } else {
          // Future instructions display configured duration
          if (inst.type == TYPE_COLOUR) {
            display.printf("%d/%d %s %s", stepIdx + 1, instructionCount, inst.functionName.c_str(),
                           (inst.colourOption == 0 ? "Light" : (inst.colourOption == 1 ? "Medium" : "Dark")));
          } else if (inst.type == TYPE_WEIGHT) {
            ...
            display.printf("%d/%d %s %s", stepIdx + 1, instructionCount, inst.functionName.c_str(), getFormattedTimeHMS(inst.durationSec).c_str());
          }
        }
        lineY += 10;
      }
    ...
    // Non-running display layout (WiFi setup / Menu / Done)
    ...
    for (int i = 0; i < instructionCount; i++) {
      if (i < currentStepIdx) {
        ...
        if (instructions[i].type == TYPE_COLOUR || instructions[i].type == TYPE_WEIGHT || instructions[i].type == TYPE_MENU || instructions[i].type == TYPE_STOP) {
          ...
          html += "<input type='text' class='time-edit' name='time' value='" + getFormattedTimeHMS(currentStepRemainingSec) + "'> ";
      ...
      if (request->hasParam("idx") && request->hasParam("time")) {
        ...
        if (idx >= currentStepIdx && idx < instructionCount) {
          ...
          if (sscanf(timeStr.c_str(), "%d:%d:%d", &h, &m, &s) == 3 && h >= 0 && m >= 0 && m <= 59 && s >= 0 && s <= 59) {
            long newDuration = (long)h * 3600 + (long)m * 60 + s;
            if (idx == currentStepIdx) {
              // Dynamically adjust active instruction's remaining duration
              currentStepRemainingSec = newDuration;
              instructions[idx].durationSec = newDuration;
            } else {
              instructions[idx].durationSec = newDuration;
            }
          }
